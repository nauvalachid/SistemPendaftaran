    ...
    <div class="flex min-h-screen bg-gray-50">
        ...
        <main class="flex-1 w-full overflow-y-auto p-6 lg:p-6 animate-fade-in-up">
            ...
            <div class="rounded-2xl bg-white p-8 shadow-sm border border-gray-100 mb-8 hover:shadow-md transition-shadow duration-300">
                ...
                {{-- ISI KONTEN ( Layouting berdasarkan kategori ) --}}
                <div class="space-y-6">
                    @foreach($kat->konten as $konten)
                        {{-- Template Visual Sederhana untuk Admin --}}
                        <div class="group border border-gray-100 p-5 rounded-xl hover:bg-gray-50 transition">
                            <div class="flex flex-col md:flex-row justify-between gap-4">
                                <div class="flex-1">
                                    <h3 class="font-bold text-lg">{{ $konten->judul }}</h3>
                                    @if($konten->sub_judul) <p class="text-blue-600 text-sm font-medium">{{ $konten->sub_judul }}</p> @endif
                                    <p class="text-gray-600 text-sm mt-2 line-clamp-3">{{ $konten->isi }}</p>
                                </div>
                                <div class="flex gap-2 items-start">
                                    <button onclick="openEditModal({{ $konten->id }})" class="bg-yellow-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-yellow-600">Edit</button>
                                    
                                    @if(!$isBeranda && !$isPPDB)
                                    <button onclick="openHapusModal('{{ route('admin.konten.destroy', $konten->id) }}')" class="bg-red-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-red-700">Hapus</button>
                                    @endif
                                    
                                    @if($isEskul)
                                    <button onclick="openMediaModal({{ $konten->id }})" class="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold">Galeri</button>
                                    @endif
                                </div>
                            </div>
