    ...
    <div class="py-12 md:py-16">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if(session('success') || session('error') || session('info') || $errors->any())
                <div class="border px-4 py-3 rounded-lg relative mb-8 max-w-2xl mx-auto text-sm @if(session('success')) bg-green-100 border-green-400 text-green-700 @elseif(session('error') || $errors->any()) bg-red-100 border-red-400 text-red-700 @else bg-blue-100 border-blue-400 text-blue-700 @endif" role="alert">
                    <strong class="font-bold">@if(session('success')) Berhasil! @elseif(session('error') || $errors->any()) Error! @else Perhatian! @endif</strong>
                    <span class="block sm:inline ml-2">@if(session('success')) {{ session('success') }} @elseif(session('error')) {{ session('error') }} @elseif(session('info')) {{ session('info') }} @else Terdapat kesalahan validasi. @endif</span>
                    @if($errors->any())<ul class="mt-2 list-disc list-inside">@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul>@endif
                </div>
            @endif
                ...
                <div class="p-6 md:p-10">
                    <form action="{{ route('pendaftaran.store') }}" method="POST" enctype="multipart/form-data" id="registration-form">
                        ...
                        <div id="step-1" class="step-content">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                ...
                                <div class="md:col-span-2">
                                    <label for="nama_siswa" class="block text-sm font-semibold text-gray-700 mb-1">Nama Lengkap *</label>
                                    <input type="text" name="nama_siswa" id="nama_siswa" value="{{ old('nama_siswa') }}" required class="form-input" placeholder="Masukkan nama lengkap">
                                ...
                                <div>
                                    ...
                                    <div class="custom-select-container" id="jenisKelaminDropdown">
                                        <input type="hidden" name="jenis_kelamin" id="jenis_kelamin_value" value="{{ old('jenis_kelamin') }}">
                                        <div class="custom-select-trigger" tabindex="0">
                                            <span id="jenisKelaminText" class="{{ old('jenis_kelamin') ? 'text-gray-800' : 'text-gray-400' }}">
                                                {{ old('jenis_kelamin') ? old('jenis_kelamin') : 'Pilih Jenis Kelamin' }}
                                            </span>
                                            <svg class="h-5 w-5 text-gray-500 arrow" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <div>
                                    <label for="tempat_tgl_lahir" class="block text-sm font-semibold text-gray-700 mb-1">Tempat, Tanggal Lahir *</label>
                                    <input type="text" name="tempat_tgl_lahir" id="tempat_tgl_lahir" value="{{ old('tempat_tgl_lahir') }}" required class="form-input" placeholder="Contoh: Yogyakarta, 20 Oktober 2018">
                                    @error('tempat_tgl_lahir') <p class="error-message">{{ $message }}</p> @enderror
                                </div>
                                
                                <div>
                                    <label for="agama" class="block text-sm font-semibold text-gray-700 mb-1">Agama *</label>
                                    <input type="text" name="agama" id="agama" value="{{ old('agama', 'Islam') }}" required class="form-input">
                                    @error('agama') <p class="error-message">{{ $message }}</p> @enderror
                                </div>
                                
                                <div>
                                    <label for="asal_sekolah" class="block text-sm font-semibold text-gray-700 mb-1">Sekolah Asal</label>
                                    <input type="text" name="asal_sekolah" id="asal_sekolah" value="{{ old('asal_sekolah') }}" class="form-input" placeholder="TK/RA asal (Opsional)">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="alamat" id="alamat" rows="4" required class="form-input" placeholder="RT, RW, Dusun, Kelurahan/Desa, Kecamatan, Kabupaten/Kota, Provinsi.">{{ old('alamat') }}</textarea>
                        ...
                        <div id="step-2" class="step-content hidden">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                <div>
                                    ...
                                    <input type="text" name="nama_ayah" id="nama_ayah" value="{{ old('nama_ayah') }}" class="form-input" placeholder="Nama lengkap Ayah">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="nama_ibu" id="nama_ibu" value="{{ old('nama_ibu') }}" class="form-input" placeholder="Nama lengkap Ibu">
                                    @error('nama_ibu') <p class="error-message">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="pendidikan_terakhir_ayah" class="block text-sm font-semibold text-gray-700 mb-1">Pendidikan Terakhir Ayah</label>
                                    <input type="text" name="pendidikan_terakhir_ayah" id="pendidikan_terakhir_ayah" value="{{ old('pendidikan_terakhir_ayah') }}" class="form-input" placeholder="Contoh: S1">
                                    @error('pendidikan_terakhir_ayah') <p class="error-message">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="pendidikan_terakhir_ibu" class="block text-sm font-semibold text-gray-700 mb-1">Pendidikan Terakhir Ibu</label>
                                    <input type="text" name="pendidikan_terakhir_ibu" id="pendidikan_terakhir_ibu" value="{{ old('pendidikan_terakhir_ibu') }}" class="form-input" placeholder="Contoh: SMA">
                                    @error('pendidikan_terakhir_ibu') <p class="error-message">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="pekerjaan_ayah" class="block text-sm font-semibold text-gray-700 mb-1">Pekerjaan Ayah</label>
                                    <input type="text" name="pekerjaan_ayah" id="pekerjaan_ayah" value="{{ old('pekerjaan_ayah') }}" class="form-input" placeholder="Contoh: Karyawan Swasta">
                                    @error('pekerjaan_ayah') <p class="error-message">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="pekerjaan_ibu" class="block text-sm font-semibold text-gray-700 mb-1">Pekerjaan Ibu</label>
                                    <input type="text" name="pekerjaan_ibu" id="pekerjaan_ibu" value="{{ old('pekerjaan_ibu') }}" class="form-input" placeholder="Contoh: Ibu Rumah Tangga">
                        ...
                        <div id="step-3" class="step-content hidden">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                <div>
                                    <label for="kk" class="block text-sm font-semibold text-gray-700 mb-1">Kartu Keluarga (KK) *</label>
                                ...
                                <div>
                                    <label for="foto" class="block text-sm font-semibold text-gray-700 mb-1">Pas Foto *</label>
                                    <input type="file" name="foto" id="foto" required class="form-file-input">
                                    @error('foto') <p class="error-message">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="ijazah_sk" class="block text-sm font-semibold text-gray-700 mb-1">Ijazah/SKL Terakhir *</label>
                                ...
                                <div class="md:col-span-2">
                                    <label for="bukti_bayar" class="block text-sm font-semibold text-gray-700 mb-1">Bukti Pembayaran (Opsional)</label>
                        ...
                        <div class="mt-10 pt-6 border-t flex items-center justify-between">
                            <button type="button" id="prev-btn" onclick="prevStep()" class="btn-secondary hidden">&laquo; Kembali</button>
                            <div id="prev-placeholder"></div>
                            <div class="flex items-center space-x-2">
                                <div id="dot-1" class="stepper-dot active"></div><div id="dot-2" class="stepper-dot"></div><div id="dot-3" class="stepper-dot"></div>
                            </div>
                            <button type="button" id="next-btn" onclick="nextStep()" class="btn-primary">Selanjutnya &raquo;</button>
